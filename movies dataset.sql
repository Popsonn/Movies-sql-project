...
--of all actors with three types of awards,for what % of them do we carry a film?
with sub1 as(select concat(aa.first_name,' ',aa.last_name)as actor_name,
...
select (count(distinct actor_name)::numeric * 100)/(select count(*)
from sub1 where number_of_awards = '3')::numeric as percentage
from sub1 join movies.film_actor fm using (actor_id)
where number_of_awards = '3'

--of all actors with two types of awards,for what % of them do we carry a film?
with sub1 as(select concat(aa.first_name,' ',aa.last_name)as actor_name,
aa.awards,aa.actor_id,
case 
 when aa.awards = 'Emmy,Oscar,Tony' then '3'
...
select (count(distinct actor_name)::numeric * 100)/(select count(*)
from sub1 where number_of_awards = '2')::numeric as percentage
from sub1 join movies.film_actor fm using (actor_id)
where number_of_awards = '2'

--of all actors with one type of award,for what % of them do we carry a film?
with sub1 as(select concat(aa.first_name,' ',aa.last_name)as actor_name,
aa.awards,aa.actor_id,
case 
 when aa.awards = 'Emmy,Oscar,Tony' then '3'
...
select (count(distinct actor_name)::numeric * 100)/(select count(*)
from sub1 where number_of_awards = '1')::numeric as percentage
from sub1 join movies.film_actor fm using (actor_id)
